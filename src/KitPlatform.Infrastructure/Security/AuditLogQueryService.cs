...
internal sealed class AuditLogQueryService : IAuditLogQuery
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!string.IsNullOrWhiteSpace(action))
            filters.Add("a.activity_action = @Action");
        if (from.HasValue)
            filters.Add("a.occurred_at >= @From");
        if (to.HasValue)
            filters.Add("a.occurred_at < @To");

        var where = string.Join(" AND ", filters);
        var sql = $"""
            SELECT COUNT(*)::int FROM kit_audit.activity_log a WHERE {where};
            SELECT
                a.id AS Id,
                a.actor_user_id AS UserId,
                u.username AS Username,
                COALESCE(a.entity_type, a.activity_type) AS EntityType,
                a.entity_id AS EntityId,
                a.activity_action AS Action,
                a.metadata::text AS PayloadJson,
