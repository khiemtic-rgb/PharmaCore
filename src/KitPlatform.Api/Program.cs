...
{
    options.SwaggerDoc("v1", new OpenApiInfo { Title = "KitPlatform", Version = "v1", Description = "ERP Nhà thuốc đa quốc gia" });
    ...
    catch (Exception ex)
    {
        ...
        var message = ex switch
        {
            ...
            _ when TryGetPostgresSqlState(ex, out var sqlState) => sqlState switch
            {
                ...
                "3D000" => "Database 'KitPlatform' chưa tồn tại. Chạy scripts\\setup-and-migrate.bat.",
                ...
                _ when IsConnectionException(ex) =>
                    "Không kết nối được PostgreSQL. Hãy bật PostgreSQL và kiểm tra database 'KitPlatform'.",
...
app.UseMiddleware<KitPlatform.Infrastructure.Security.WorkspaceResolutionMiddleware>();
    ...
    CustomerAppAuthSettings customerAppAuth)
{
    ...
    if (string.IsNullOrWhiteSpace(connectionString)
        || connectionString.Contains("KitPlatform_dev", StringComparison.OrdinalIgnoreCase))
    {
        throw new InvalidOperationException(
