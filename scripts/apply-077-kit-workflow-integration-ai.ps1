...
$MigrationFile = Join-Path $Root "migrations\077_kit_workflow_integration_ai.sql"
...
if (-not $psql) {
    Write-Host '[CANH BAO] Khong tim thay psql — bo qua migration 077.' -ForegroundColor Yellow
    exit 0
}
...
Write-Host ">> KIT Platform kernel Phase 6 (final): 077_kit_workflow_integration_ai.sql" -ForegroundColor Yellow
& $psql $ConnectionString -v ON_ERROR_STOP=1 -f $MigrationFile
if ($LASTEXITCODE -ne 0) {
    Write-Host "[LOI] Migration 077 that bai." -ForegroundColor Red
    exit $LASTEXITCODE
}
...
$missingCount = & $psql $ConnectionString -t -A -c "SELECT COUNT(*)::text FROM kit_core.kernel_table_registry WHERE registry_status = 'MISSING'"
$phase = & $psql $ConnectionString -t -A -c "SELECT kernel_phase FROM kit_core.platform_kernel_version WHERE id = 1"
Write-Host "[OK] Migration 077 applied. Phase: $phase | EXISTS: $existsCount | VIEW: $viewCount | MISSING: $missingCount" -ForegroundColor Green
