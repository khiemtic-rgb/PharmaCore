...
import { batchLabelMatchesHints } from '@/modules/sales/pos-batch';
...
function resolveBatchNumber(scan: string, line: CartLine): string | null {
  ...
  for (const normalized of candidates) {
    ...
    if (matches.length === 1) return matches[0]!.batchNumber;
    ...
    if (matches.length > 0) return matches[0]!.batchNumber;
    return normalized;
  }
  return null;
}
...
export function applyBatchLabelScan(
  ...
  return {
    cart: cart.map((line) => (line.key === target.key ? { ...line, batchLabel: batchNumber } : line)),
    batchNumber,
