-- KitPlatform 098: track applied production migrations (incremental deploy)
-- Backfills migrations already live on existing VPS databases.

CREATE TABLE IF NOT EXISTS public.kit_schema_migrations (
    filename    TEXT PRIMARY KEY,
    applied_at  TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

COMMENT ON TABLE public.kit_schema_migrations IS
    'Production migration ledger — one row per applied file from migration-files.prod.txt';

-- Backfill historical migrations on databases that predated this ledger.
...
INSERT INTO public.kit_schema_migrations (filename, applied_at)
VALUES ('098_schema_migrations.sql', NOW())
ON CONFLICT (filename) DO NOTHING;
